...
lettuces-own [timecreated timevalue]
...
globals [
  ...
  last-count ]  ;; this is the number of nuclei at the beginning -- reset each time a red line is drawn on the plot
...
to setup
  ...
  set FC-0 20
  ;set decay-chance ( percentage 20 )
  set counter 1
  set FC FC-0
...
to go
  start-timer
  Change-chlorine-Level
  move-lettuce-in-the-washer
  to-create-lettuces
  chlorineprocess
  ask lettuces with [ pxcor >= 15] [ die ]
  Change_Decay_Based_On_Mass
  tick
end

to Change-Chlorine-Level            ;; The amount of free chlorine is change after
 ifelse mod-28-number_Of_processes =  0 and time4 = 1
      [ if counter = 1 [ ask n-of floor ( count patches * 13.08 / 20 ) patches [set pcolor cyan ]]
        if counter = 2 [ ask n-of floor ( count patches * 7.18 / 20 ) patches [set pcolor cyan ]]
        if counter = 3 [ ask n-of floor ( count patches * 5.08 / 20 ) patches [set pcolor cyan set counter 0 ]]
        set counter counter + 1 ]
     [ set DummyVariable DummyVariable + 1 ]
end

to Change_Decay_Based_On_Mass
 if ( number_Of_processes != counter_1 ) [
...
to to-create-lettuces
  if count lettuces with [ pxcor <= -15]  = 0 [ ask patches with [ pxcor <= -15]
    [sprout-lettuces  2 [set color blue set shape "circle" set Batch-26s  Batch-26s + 1  ] ] ]
end

to chlorineprocess
  let processed_mass precision ( ( number_Of_processes  ) * (19526 / 45000) * 45  ) 2
  set COD COD +  0.65873 * ( number_Of_processes  ) * (19526 / 45000)* 45
  set FC-0 FC-0 + TimeVariable
  set  FC ( FC-0 ) * e ^ ( -0.0093705 *  Processed-Mass )
end


to keep_track_Of_COD
   if  (pxcor <= -15 and any? lettuces-here) [ set Batch-26s  Batch-26s + 1 ]
end

to-report   Processed-Mass
  report ( ( number_Of_processes  ) * (19526 / 45000) * 45  )
end

to start-timer

  ifelse mod-28-number_Of_processes = 0 [ set time1 ticks   ]
  [ set time1 0  ]

  ifelse mod-28-number_Of_processes != 0  [  set time2 ticks
  set time3  time2 - time1][ set time2 0 ] ; set time3 0

  ;;if number_Of_processes = 0 [set time ticks ]
end

to-report deplition_by_percentage
  report  count patches  with [pcolor = cyan] / count patches * 20
end
...
to-report number_Of_processes
  report  floor ( Batch-26s / 210  )
end

to-report mod-28-number_Of_processes
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy Cumulative-Mass ( count patches  with [pcolor = cyan] / count patches * 20 )"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy ( ( Batch-26s / 210 ) * 45 / 2.3 ) FC"

...
to draw-walls-2
  ...
  if (window-edge > (min-pycor + shrink))
    [ set window-edge (min-pycor + shrink) ]
  ask patches with [(pxcor = min-pxcor) or
    ((pxcor <= max-pxcor) and (abs pycor = max-pycor)) or
                   (pxcor >= max-pxcor - shrink) or
                  ((pxcor >= max-pxcor) and (abs pycor >= max-pycor - shrink)) or (pycor = min-pycor)]
  [ set pcolor green ]
...
;; positive) or expand (if amount is negative)
to shrink-or-expand-left [amount]
  set shrink (shrink + amount)
  ;; keep it within bounds
  if (shrink < 0)
    [ set shrink 0 ]
  if (shrink <= min-pxcor + 2)
    [ set shrink min-pxcor + 2 ]
  if (shrink <= min-pycor + 2)
    [ set shrink min-pycor + 2 ]
  clear-patches
  draw-walls-2
  ;; make sure no turtles are embedded in the walls
  ask turtles with [(xcor > 0) and (pcolor = green)]
...
* Stieff, M. and Wilensky, U. (2001).  NetLogo Simple Kinetics 3 model.  http://ccl.northwestern.edu/netlogo/models/SimpleKinetics3.  Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.

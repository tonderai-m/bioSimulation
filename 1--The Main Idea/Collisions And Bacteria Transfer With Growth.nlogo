...
to make-particles
  create-particles initial-number-particles [
    ...
    ;; set the CFU proportional to the area of the particle
    set CFU 10 ^ InputBacteria
    set Q 0.0064 * Temperature ^ 2 - 0.205 * Temperature + 0.1647
...
to-report overlapping?  ;; particle procedure
  ...
  ;; is done by DISTANCE
  report any? other particles in-radius size
...
to collide-with [other-particle]  ;; particle procedure
  ...
  let CFU2 [CFU] of other-particle
  ...
  ;; compute the velocity of the system's center of CFU along theta
  let vcm (((CFU * v1t) + (CFU2 * v2t)) / (CFU + CFU2) )
 ...
 if Cross_Contamination?
    [ let  Lettuce-to-Worker random-triangle 0 0.01 0.03
      ...
      repeat  CFU [ if random-float 1 < Lettuce-to-Worker [set bin_ct bin_ct + 1] ]
        ask other-particle [  repeat   CFU [ if random-float 1 < Worker-to-Lettuce [set bin_ct1 bin_ct1 + 1 ] ] ]
      set   CFU  CFU - bin_ct + bin_ct1
        ask other-particle [ set    CFU   CFU  - bin_ct1 + bin_ct ]
      set bin_ct 0
      set bin_ct1 0 ]
...
to-report max-particle-CFU
  report max [CFU] of particles
end

to-report kinetic-energy
   report (0.5 * CFU * speed * speed)
end
...
to show-labels
   ask turtles [ ifelse Show-Surface_Bacteria [ set label precision CFU  2] [ set label  CFU  ] ]
end

to Calclculate-TimeIn-System
  ask turtles [ set timeInSystem-h  Hour-storage]
end


to Grow-Bacteria_II
  if  Calculated-Time = 0 and ticks > 1 [ ask turtles  [
   set logCFU log CFU 10
   let step_size 1
   set Q Q + (( k1 + (2 * k2)+( 2 * k3) + K4 ) * step_size / 6)
   set Umax  ( 0.0169 * (Temperature + 4.012) ) ^ 2
   set LogCFU_Max  8.676 / (1 + exp ( - ( Temperature - 3.52 ) / 2.876 ))
   set k1  precision ( ( Umax / ( 1 + exp( - Q))) * ( 1 - exp( logCFU - LogCFU_Max)) )9
   set k2  precision ( ( Umax / ( 1 + exp(-( Q +( k1 * timeInSystem-h / 2))))) * ( 1 - exp( logCFU + ( k1 * step_size / 2) - LogCFU_Max)) )9
   set k3  precision ( ( Umax / ( 1 + exp(-( Q +( k2 * step_size / 2))))) * ( 1 - exp( logCFU + ( k2 * step_size / 2) - LogCFU_Max))) 9
   set k4  precision ( ( Umax / ( 1 + exp(-( Q +( k3 * step_size ))))) * ( 1 - exp( logCFU + ( k3 * step_size) - LogCFU_Max))) 9
...
to Calculate-Time
  if  Calculated-Time = 0 and ticks > 1 [ Set  Time-Counter  Time-Counter + 1 ]
end

to-report Calculated-Time
report floor ticks mod 19
end
to-report Hour-storage
report Time-Counter
end
to-report Bacteria-Amount
report sum [CFU] of turtles
end

to Facility-Temperature
  set Temperature Set-Temperature + Temp-Offset * sin (  Hour-storage * 180 / Temp-Oscillation-Speed)
end
...
;      let Worker-to-Lettuce  random-triangle 0.03 0.10 0.3
;      repeat  CFU [ if random-float 1 < Lettuce-to-Worker [set bin_ct bin_ct + 1] ]
;      repeat   CFU [ if random-float 1 < Worker-to-Lettuce [set bin_ct1 bin_ct1 + 1 ] ]
;      set   CFU  CFU - bin_ct + bin_ct1
;      set    CFU   CFU  - bin_ct1 + bin_ct
;      set bin_ct 0
;      set bin_ct1 0 ]]
;end
;
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy Hour-storage Bacteria-Amount "
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy Hour-storage Temperature"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy  Hour-storage log Bacteria-Amount  10"
